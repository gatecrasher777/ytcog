...
let app = {
	...
	ignore: ['cookie', 'userAgent', 'options', 'sapisid', 'status', 'reason', 'cancelled','debugOn','commentOptions','commentOrder','defaultCaptions','formnats'],
	// video options
	test_options: {
		// any video id
		id: '5qwDrjTinMk',
		...
		// make subtitles if captions are available, chose language codes, comma separated
		subtitles: 'en,es,ja',
		// chose the subtitle file format, one or more of srt, vtt or ttml
		subtitleFormat: 'srt,vtt,ttml',
...
async function runOnline(video) {
	console.log('\nCheck online status via preview image');
	await video.imageOnline();
	console.log(`Video status: ${video.status} (${video.reason})`);
}

async function runInfo(video) {
...
async function runComments(video) {
...
async function runDownloads(video) {
