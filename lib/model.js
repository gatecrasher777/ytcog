...
class Model {
	...
	constructor(cookie, userAgent, minigetOptions) {
		this.type = 'model';
		this.cookie = '';
		this.userAgent = ut.ff();
		this.minigetOptions = {};
		if (typeof cookie === 'object') {
			this.minigetOptions = cookie;
		} else if (typeof userAgent === 'object') {
			this.cookie = cookie;
			this.minigetOptions = userAgent;
		} else if (typeof minigetOptions === 'object') {
			this.cookie = cookie;
			this.userAgent = userAgent;
			this.minigetOptions = minigetOptions;
		} else {
	...
	// merge options
	combine(method, options, extra, compress) {
		let hdrs, opts;
		options.headers ? hdrs = { ...this.common.headers, ...options.headers } : hdrs = this.common.headers;
		if (extra.headers) hdrs = { ...hdrs, ...extra.headers };
		opts = { ...this.common, ...options, ...extra };
		opts.method = method;
		opts.headers = hdrs;
		if (compress) {
	...
	async httpsPost(url, postOptions, postData, compress) {
		let body;
		try {
			let opts = this.combine('POST', postOptions, this.minigetOptions, compress);
			let req = miniget(url, opts);
			if (typeof this.minigetOptions.requestCallback === 'function') this.minigetOptions.requestCallback(req);
			let uncompressed = 0;
			let compressed = 0;
			req.once('request', ar => {
				ar.write(postData);
	...
	async httpsGet(url, getOptions, compress) {
		...
		try {
			let uncompressed = 0;
			let compressed = 0;
			let opts = this.combine('GET', getOptions, this.minigetOptions, compress);
			let req = miniget(url, opts);
			if (typeof this.minigetOptions.requestCallback === 'function') this.minigetOptions.requestCallback(req);
			req.on('data', chunk => {
				uncompressed += chunk.length;
	...
	async httpsHead(url) {
		try {
			let headOptions = this.combine('HEAD', {
				headers: {
					origin: 'https://www.youtube.com',
					referer: 'https://www.youtube.com/',
					'user-agent': this.userAgent,
				...
				maxReconnects: 3,
				backoff: { inc: 500, max: 10000 },
			}, this.minigetOptions, false);
			let req = miniget(url, headOptions);
			if (typeof this.minigetOptions.requestCallback === 'function') this.minigetOptions.requestCallback(req);
			await req.text();
			this.status = 'OK';
			this.reason = '';
