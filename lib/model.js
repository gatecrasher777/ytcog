	...
	get info() {
		...
		props.forEach((k)=>{
			switch (typeof this[k]) {
				...
				case 'string':
					if (this[k].length) {
						if (!['data'].includes(k)) meta[k]=this[k];
					}
					break;
				case 'number':
					if (this[k]>0) {
						if (['published','updated','downloaded'].includes(k)) {
							meta[k]=ut.tsAge(this[k]); 
						}
					}
					break;
				case 'object':
					if ((!['videos','player'].includes[k]) && (Array.isArray(this[k])) && (this[k].length)) {
						meta[k]=this[k];
					}
					break;
				default: 
