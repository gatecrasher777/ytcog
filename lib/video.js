...
class Video extends Model {
	...
	async fetch() {
		...
		let postOptions = {
			...
			headers: {
				...
				'authorization' : this.session.player.idhashFn(this.sapisid)
			}
		}
		let body = await this.httpsPost('https://www.youtube.com/youtubei/v1/player?key='+this.session.key,postOptions,postData,true);
		if (body) {
	...
	async #downloadInstance(vformat, aformat, vonly, aonly) { 
		try {
			console.log('dl instance '+vformat+' '+aformat+' '+vonly+' '+aonly);
			let vs, as, uc, uq;
			let data = {
				id: this.options.id,
