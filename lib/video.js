	...
	async makeSRT(url, code, translate) {
		...
		if (body) {
			...
			result.events.forEach(event => {
				c++;
				let start = parseInt(event.tStartMs);
				let end = start + parseInt(event.dDurationMs);
				let text = '';
	...
	async subtitles() {
		if (this.hasCaptions && this.fn.length && this.options.subtitles !== 'none') {
			...
			langs.forEach(async language => {
				...
				let found = false;
				this.captions.forEach(async caption => {
					if (code === caption.code) {
						await this.makeSRT(caption.url, code, false);
						found = true;
					}
				});
				if (!found) {
					let caption = this.captions[this.defaultCaptions];
					await this.makeSRT(caption.url, code, true);
				}
			});
		}
	}
