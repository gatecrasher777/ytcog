...
class Channel extends Model {
	...
	constructor(session, options) {
		...
		this.spmap = {
			...
			'shorts,new': 'EgZzaG9ydHPyBgUKA5oBAA%3D%3D',
			...
			items.forEach(item => {
				...
				if (w && w.videoId) {
					...
					try {
						this.quantity++;
						let ts = 0;
						let video = new Video(this.session, { id: s.videoId, published: ts });
						if (s.headline) video.title = s.headline.simpleText;
						if (s.viewCountText) video.views = ut.viewQ(s.viewCountText.simpleText);
						let ac = s.accessibility.accessibilityData.label;
						let ex = ac.indexOf(' seconds - play video');
						video.duration = parseInt(ac.substring(ex-2,ex));
						video.author = this.author;
						video.channelId = this.id;
						video.channelThumb = this.thumbnail;
						video.country = this.country;
				...
				if (tabs) {
					tabs.forEach(tab => {
						if (tab.expandableTabRenderer && this.options.items === 'search') {
							...
							if (this.options.items === 'about') {
								try {
									...
									if (tabs) {
										this.tabs = [];
										tabs.forEach(tb => {
											let tr = tb.tabRenderer;
											if (!tr) tr = tb.expandableTabRenderer;
											if (tr) this.tabs.push(tr.title);
										});
									}
								} catch (e) {
									this.debug(e);
							...
							} else if (this.options.items === 'videos' || this.options.items === 'shorts') {
								try {
									gi = tab.tabRenderer.content.richGridRenderer.contents;
								} catch(e) {
									this.debug(e);
									this.status = 'OK';
									this.reason = 'No results';
