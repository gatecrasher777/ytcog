...
class Download {
	...
	constructor(options, cookie, userAgent, proxy, debug = false) {
		this.session = new Session(cookie, userAgent, proxy);
		this.video = null;
...
const ytcog = {
	...
	Video: Video,
	Download: Download,
	dl: async(options, cookie, userAgent, proxy, debug = false) => {
		let download = new Download(options, cookie, userAgent, proxy, debug);
