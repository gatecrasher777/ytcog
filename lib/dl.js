...
class Download {
	...
	nextVideoSection() {
		if (!this.cancelled) {
			...
			let end;
			if (this.maxSection) {
				start = this.curSection * 1024 * 1024 * this.vSize;
				end = this.curSection === this.maxSection ? this.data.stream.vsize : start + (1024 * 1024 * this.vSize) - 1;
			} else {
				start = 0;
				end = this.data.stream.vsize - 1;
			}
			this.opts.headers.Range = `bytes=${start}-${end}`;
			this.vrs = miniget(`${this.data.stream.video_url}&range=0-${this.data.stream.vsize}`, this.opts);
			this.tmo = setTimeout(this.timedOut.bind(this), this.timeout);
	...
	nextAudioSection() {
		if (!this.cancelled) {
			...
			let end;
			if (this.maxSection) {
				start = this.curSection * 1024 * 1024 * this.aSize;
				end = this.curSection === this.maxSection ? this.data.stream.asize : start + (1024 * 1024 * this.aSize) - 1;
			} else {
				start = 0;
				end = this.data.stream.asize - 1;
			}
			this.opts.headers.Range = `bytes=${start}-${end}`;
			this.ars = miniget(`${this.data.stream.audio_url}&range=0-${this.data.stream.asize}`, this.opts);
			this.tmo = setTimeout(this.timedOut.bind(this), this.timeout);
