...
class Download {
    ...
    nextVideoSection() {
        if (!this.cancelled) {
            if (this.maxSection) {
                let start = this.curSection*1024*1024*10;
                if (this.curSection == this.maxSection) {
    ...
    // download video, determine number of sections keeping each section <= 10MB
    vdownload() {
        this.type = 'video';
        ...
        if (this.data.stream.asize == -1) { //combined video/audio stream - do not section
          ...
          this.maxSection = Math.floor(this.data.stream.vsize/1024/1024/10);
        }
        this.curSection = 0;
        this.nextVideoSection();
    }
    ...
    nextAudioSection() {
        if (!this.cancelled) {
            if (this.maxSection) {
                let start = this.curSection*1024*1024*1;
                if (this.curSection == this.maxSection) {
                    this.opts.headers['Range'] =`bytes=${start}-`;
                } else {
    ...
    // download audio, determine the number of sections keeping each section <= 1MB
    adownload() {
        this.type = 'audio';
        this.aws = fs.createWriteStream(this.afn);
        this.maxSection = Math.floor(this.data.stream.asize/1024/1024/1);
        this.curSection = 0;
        this.nextAudioSection();
    }
