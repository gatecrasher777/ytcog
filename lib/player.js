...
class Player extends Model {
	...
	extractNCode() {
		console.log('extractNCode: 1');
		let fname = ut.pinch(this.data, '&&(b=a.get("n"))&&(b=', '(b)', 0, 0, '', false);
		console.log('extractNCode: 2');
		if (fname.indexOf('[') !== -1) {
			fname = ut.pinch(this.data, `${fname.split('[')[0]}=[`, ']', 0, 0, '', false);
		}
		console.log('extractNCode: 3');


		let ffunc = `${fname}=function(a)`;
		console.log('extractNCode: 4');

console.log(`\n\n${this.data}\n\n`);

		ffunc = `var ${ut.block(this.data, ffunc, 0, '{', '}', '{}', false)}`;
		console.log('extractNCode: 5');
...
console.log(`\n\n${ffunc}\n\n`);
		...
		let ncodeScript;
		try {
			ncodeScript = new vm.VMScript(`( function(ytcogncode) {${ffunc}} )`).compile();
		}
		catch (e) {
		...
		console.log('extractNCode: 6');

		this.ncodeFn = this.context.run(ncodeScript);
		console.log('extractNCode: 7');
