...
class Player extends Model {
	...
	constructor(session) {
		...
		this.decipherScript = null;
		this.ncodeScript = null;
		this.idhashScript = null;
		this.decipherFn = sig => this.decipherScript.runInNewContext({ ytcogsig: sig });
		this.ncodeFn = n => this.ncodeScript.runInNewContext({ ytcogncode: n });
		this.idhashFn = id => this.idhashScript.runInNewContext({ ytcogsapisid: id });
	}
	...
	extractDecipher() {
		...
		ffunc = `${this.extractManipulations(ffunc)};${ffunc};${fname}(ytcogsig);`;
		this.decipherScript = new vm.Script(ffunc);
	}
	...
	extractNCode() {
		...
		let ffunc = `${fname}=function(a)`;
		ffunc = `var ${ut.block(this.data, ffunc, 0, '{', '}', '{}', false)}`;
		this.ncodeScript = new vm.Script(`${ffunc}; ${fname}(ytcogncode);`);
	}
	...
	extractIdHash() {
		...
		ffunc = `var ${ut.block(this.data, ffunc, 0, '{', '}', '{}', false)}`;
		this.idhashScript = new vm.Script(`${ffunc};
