...
class Player extends Model {
	...
	extractDecipher() {
        ...
        let ffunc = 'var '+ut.block(this.data,ffunc,'{','}',0,'{}',false);
        let ffunc = this.extractManipulations(ffunc)+';'+ffunc+'; return '+fname+'(sig);';
    ...
    extractIdHash(body) {
        ...
        let ffunc = 'var '+ut.block(this.data, ffunc, 0, '{','}','{}',false);
        this.idhashFn = new Function(id, ffunc+`;
    ...
    process() {
        try {
            ...
            this.extractNCode(body);
            this.extractIdHash(body);
	...
	async fetch() {
        this.data = await this.httpsGet(this.item.meta.url,{
            headers: {
