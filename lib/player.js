...
class Player extends Model {
	...
	extractDecipher() {
		...
		ffunc = `${this.extractManipulations(ffunc)};${ffunc};${fname}(sig);`;
		this.decipherFn = new vm.Script(ffunc);
	}
	...
	extractNCode() {
		...
		let ffunc = `${fname}=function(a)`;
		ffunc = `var ${ut.block(this.data, ffunc, 0, '{', '}', '{}', false)}`;
		this.ncodeFn = new vm.Script(`${ffunc}; ${fname}(ncode);`);
	}
	...
	extractIdHash() {
		...
		ffunc = `var ${ut.block(this.data, ffunc, 0, '{', '}', '{}', false)}`;
		this.idhashFn = new vm.Script(`${ffunc};
	...
	decipher(url) {
		...
		let sig = this.decipherFn.runInNewContext({ sig: decodeURIComponent(args.s) });
	...
	ncode(url) {
		...
		if (!n) return url;
		n = this.ncodeFn.runInNewContext({ ncode: n });
