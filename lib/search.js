...
class Search extends Model {
	...
	get #params() { 
		let sp = '';
		let code = '';
		(this.options.period === undefined) ? sp = 'any,' : sp = this.options.period+',';
		(this.options.length === undefined) ? sp += 'any,' : sp += this.options.length+',';
		(this.options.order === undefined) ? sp += 'relavance' : sp += this.options.order;
		code = this.spmap[sp];
		if (code == undefined) code = this.spmap["any,any,relevance"];
		return code;
	}
